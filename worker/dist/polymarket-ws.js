...
class PolymarketWSManager {
    ...
    async resolveAllSlugs() {
        ...
        await Promise.allSettled(slugs.map(async (slug) => {
            try {
                const res = await (0, retry_1.fetchWithRetry)(`https://gamma-api.polymarket.com/markets?slug=${encodeURIComponent(slug)}&limit=1`, { headers: { accept: 'application/json' }, signal: AbortSignal.timeout(8000) });
